...
// Design: Tactical Dark Ops — compact confirmation dialog with destructive red accent
// Shown before deleting a character to prevent accidental deletion
...
interface DeleteConfirmDialogProps {
  ...
  onConfirm: (character: ApiCharacter) => void;
  onCancel: () => void;
  isDeleting?: boolean;
}
...
export default function DeleteConfirmDialog({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && character) onCancel();
    };
    ...
    >
      ...
      >
        ...
        <div className="px-5 py-5">
          ...
          <div className="flex items-center gap-3 mb-4 p-3 rounded-sm" style={{ background: 'oklch(0.15 0.01 264)', border: '1px solid oklch(1 0 0 / 0.07)' }}>
            {(character.display_headshot_url || character.headshot_url) && (
              <img
                src={character.display_headshot_url ?? character.headshot_url ?? ''}
                alt={character.name}
          ...
          >
            This action is <span style={{ color: 'oklch(0.75 0.18 25)', fontWeight: 600 }}>permanent</span> and cannot be undone. The character and all associated data will be removed.
          </p>
        </div>
