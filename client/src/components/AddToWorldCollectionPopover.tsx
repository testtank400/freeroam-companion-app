// AddToWorldCollectionPopover.tsx
// Popover for adding/removing a world from world collections.
// Shows a searchable list of collections with toggle membership.
...
interface AddToWorldCollectionPopoverProps {
  worldExternalId: string;
  collections: ApiWorldCollection[];
  /** Set of collection IDs that this world belongs to */
...
export default function AddToWorldCollectionPopover({
  worldExternalId,
  collections,
  membershipSet,
  onToggle,
  onCreate,
  onClose,
}: AddToWorldCollectionPopoverProps) {
  ...
  const handleToggle = async (collectionId: string) => {
    ...
    try {
      if (isIn) {
        await removeMutation.mutateAsync({ collectionId, worldExternalId });
        onToggle(collectionId, false);
      } else {
        await addMutation.mutateAsync({ collectionId, worldExternalId });
        onToggle(collectionId, true);
      }
    } catch (err) {
