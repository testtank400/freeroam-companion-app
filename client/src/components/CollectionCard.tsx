...
export default function CollectionCard({ collection, characters, onClick, onEdit, onDelete }: CollectionCardProps) {
  ...
  return (
    <div
      ...
      onClick={() => { if (!confirmingDelete) onClick(collection); }}
    >
      {/* Image area */}
      <div className="relative w-full" style={{ paddingBottom: '115%' }}>
        ...
        <div className="absolute top-2.5 right-2.5 z-10 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {/* Delete: trash icon → confirmation buttons */}
          {confirmingDelete ? (
            <div
              className="flex items-center gap-1 px-2 py-1 rounded-sm"
              style={{
                background: 'oklch(0.18 0.01 264 / 0.95)',
            ...
            >
              ...
              <button
                onClick={(e) => { e.stopPropagation(); onDelete(collection); setConfirmingDelete(false); }}
                className="px-2 py-0.5 rounded text-[10px] font-bold uppercase transition-all hover:brightness-125"
                style={{ background: 'oklch(0.65 0.22 25)', color: 'white', fontFamily: 'Rajdhani, sans-serif' }}
              >
                Yes
              </button>
              <button
                ...
                className="px-2 py-0.5 rounded text-[10px] font-bold uppercase transition-all hover:brightness-125"
                style={{ background: 'oklch(0.25 0.01 264)', color: 'oklch(0.6 0.01 264)', fontFamily: 'Rajdhani, sans-serif' }}
              >
                No
              </button>
            </div>
